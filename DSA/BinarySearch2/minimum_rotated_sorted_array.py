class Solution:
    def findMin(self, nums: List[int]) -> int:
        ...
        while left <= right:
            ...
            if (
                nums[mid] > nums[0]
            ):  # if mid element is greater than first element, search right side
                left = mid + 1
            else:  # if mid element is lesser than first element, search left side
                right = mid - 1

            # if the mid element is greater than next element,
            # thats where rotation happens so mid+1 element is smallest
